...
class ID900TimeTagger(TimeTaggerInstrument):
	...
	def measure_correlation(self, start_ch: int, stop_ch: int, integration_time_s: float = 1.0, coinc_window_ps: int = 1000) -> int:
		...
		self._tagger.send_string(SCPIcommand)
		dela6_val = int(self._tagger.recv().decode('utf-8')) + coinc_window_ps
		...
		self._tagger.send_string(f'TSCO1:WIND:ENAB ON;BEGI:DELA {start_time};EDGE RISING;LINK DELA5:TSCO1:WIND:END:DELA {end_time};EDGE RISING;LINK DELA5;:TSCO1:FIR:LINK DELA6;:TSCO:SEC:LINK NONE;:TSCO1:OPIN ONLYFIR;OPOUT MUTE;:TSCO1:COUN:INTE {count_time_ms};MODE CYCLE')
		ans = self._tagger.recv().decode('utf-8')
		
		self._tagger.send_string('TSCO1:COUN:RESET')
		ans = self._tagger.recv().decode('utf-8')
		
		self._tagger.send_string('TSCO1:INTE?')
		answer = self._tagger.recv().decode('utf-8')
		return int(answer)
